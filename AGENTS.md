...
The files in @internal/engine/ correspond directly to rules in the ANTLR grammar (@internal/grammar/antlrGen/Fhirpath.g4). Each file implements the evaluation logic for its corresponding grammar rule. Sometimes there are helper functions in other files than listed above that should be called from other grammar engine rules.

- @internal/engine/dynamicValue.go - Runtime value representation.
- @internal/engine/engine.go - Core engine initialization and coordination.
- @internal/engine/utils.go - Utility functions.
...
- Grammar Changes: If modifying FHIRPath language support, update @internal/grammar/antlrGen/Fhirpath.g4 first and regenerate the parser. This will never happen in the context of AI.
- Engine Files: Create or modify files in @internal/engine/ that correspond to the grammar rules being implemented.
- Naming Convention: File names should match the grammar rule names (e.g., `memberInvocation` rule @internal/engine/MemberInvocation.go).
